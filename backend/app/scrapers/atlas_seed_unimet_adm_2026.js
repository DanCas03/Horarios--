...
const SUBJECTS_RAW = [
  ["FBTGF01", "Introduccion a las Ciencias Administrativas", 1, "obligatoria", null],
  ...
  ["BPTGF22", "Teoria del Comportamiento Organizacional", 5, "obligatoria", "BPTBC30"],
  ["FBTEP02", "Mundo Global Tendencias y Transformaciones", 6, "electiva", null],
  ...
  ["FPTEJ20", "Derecho para los Negocios (Business Law)", 10, "obligatoria", null],
  ...
  ["FPTMK04", "Gerencia de Mercadeo y Ventas", 11, "mencion_gerencia", "BPTMK01"],
  ["FPTGF10", "Negociacion", 11, "mencion_gerencia", "105 creditos"],
  ["FPTMK20", "Mercado Internacional", 12, "mencion_gerencia", "FPTMK01"],
  ["FPTGF53", "Manufactura y Cadena de Suministros", 12, "mencion_gerencia", "FPTGF10"],
  ["FPTGF11", "Procesos Gerenciales", 12, "mencion_gerencia", "FPTGF10"],
  ["FPTBC56", "Gestion de Tesoreria", 11, "mencion_banca_finanzas", "105 creditos"],
  ["FPTBC76", "Banca y Seguros", 11, "mencion_banca_finanzas", "105 creditos"],
  ["FPTGF21", "Analisis de Inversion y Portafolio", 12, "mencion_banca_finanzas", "BPTGF02"],
  ["FPTGF24", "Simulacion Financieras", 12, "mencion_banca_finanzas", "BPTGF02"],
  ["FPTGF22", "Productos Derivados", 12, "mencion_banca_finanzas", "BPTGF02"],
];

function buildSubject([code, name, semester_suggested, subject_type, prelacion]) {
  const isCodeLike = typeof prelacion === "string" && /^[A-Z]{2,5}[A-Z0-9-]{2,}$/.test(prelacion);
  return {
    code,
    name,
    credits: 4,
    ...
    description: prelacion && !isCodeLike ? `Prelacion (segun flujograma): ${prelacion}` : null,
  };
}
...
if (!adm) {
  const insertResult = careers.insertOne({ ...ADM_CAREER, university_id: universityId });
  adm = careers.findOne({ _id: insertResult.insertedId });
  print(`[OK] Carrera creada: ${adm._id}`);
} else {
  careers.updateOne({ _id: adm._id }, { $set: { ...ADM_CAREER, university_id: universityId } });
  print(`[OK] Carrera actualizada: ${adm._id}`);
}

const careerId = String(adm._id);

const deleted = subjects.deleteMany({ university_id: universityId, career_id: careerId });
...
if (docs.length > 0) {
  ...
  print(`[OK] Materias insertadas: ${Object.keys(inserted.insertedIds).length}`);
} else {
